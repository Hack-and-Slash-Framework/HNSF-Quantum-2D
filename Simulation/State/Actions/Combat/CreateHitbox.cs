...
{
    [Serializable]
    public unsafe partial class CreateHitbox : HNSFStateAction
    {
            ...
            ref HNSFStateContext stateContext)
        {
            ...
            if (boxCombatant->HitboxExistWithId(frame, hitboxIdentifer))
            {
                Log.Debug($"Hitbox of id {hitboxIdentifer} already exist on entity {entity.ToString()}. Error came from state {frame.FindAsset<HNSFState>(stateContext.workingState).Label}");
                return false;
            }
            ...
            {
                Layer = frame.Layers.GetLayerIndex("Hitbox"),
                IsTrigger = true,
                Shape = shape
            };
        ...
        {
            return CopyTo(new CreateHitbox());
        }

        public override HNSFStateAction CopyTo(HNSFStateAction target)
        {
            var t = target as CreateHitbox;
