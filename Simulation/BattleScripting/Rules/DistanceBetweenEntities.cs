...
namespace HnSF.core.GroupControl.Grabbers
{
    ...
    public unsafe partial class DistanceBetweenEntities : GroupControlRule
    {
        ...
        public override bool IsValid(Frame frame, EntityRef infoEntityRef, ref BattleScriptContext context)
        {
            ...
            if(!frame.Unsafe.TryGetPointer<Transform2D>(entityA, out var transformA)
               || !frame.Unsafe.TryGetPointer<Transform2D>(entityB, out var transformB)) return false;

            var val = FPVector2.DistanceSquared(transformA->Position, transformB->Position);
            bool result = false;
            
            switch (comparison)
            {
                case ComparisonType.Inbetween:
                    result = val >= min * min && val <= max * max;
                    break;
...
{
    [Serializable]
    internal class ConditionDistanceBetweenEntities : RuleNodeBase
    {
        ...
        public override GroupControlRule Convert()
        {
            ...
            return new Grabbers.DistanceBetweenEntities()
            {
                Label = label,
