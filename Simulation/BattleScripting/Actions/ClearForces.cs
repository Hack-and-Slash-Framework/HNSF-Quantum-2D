    ...
    public unsafe partial class ClearForces : GroupControlAction
    {
        ...
        public override bool Tick(Frame frame, EntityRef infoEntityRef, ref BattleScriptContext context)
        {
            foreach (var entityRefFunction in entityRefFunctions)
            {
                ...
                {
                    physics->force = FPVector2.Zero;
                    physics->SetExternalImpulse(frame, targetEntity, FPVector2.Zero);
                }

                if (frame.Unsafe.TryGetPointer<KCC2D>(targetEntity, out var kcc2d))
                {
                    kcc2d->_dynamicVelocity = FPVector2.Zero;
                    kcc2d->_kinematicVelocity = FPVector2.Zero;
                }
            }
            return true;
        }
